...
public class TransferenciaControllers {
    ...
    public static boolean uploadFolderContent(ChannelSftp remote, File[] localFiles) {
        for (File localFile : localFiles) {
            ...
            // Cambiar la extensión de ".json" a ".pdf"
            if (filename.endsWith(".json")) {
                String newFilename = filename.substring(0, filename.lastIndexOf('.')) + "json.pdf";

                if (IN_DEVELOPMENT) {
                    Log.debug("Changing file extension from <{0}> to <{1}>", filename, newFilename);
                }

                File renamedFile = new File(localFile.getParent(), newFilename);
                boolean renamed = localFile.renameTo(renamedFile);

                if (!renamed) {
                    if (IN_DEVELOPMENT) {
                        Log.error("File extension change failed for <{0}>", filename);
                    }
                    return false;
                }
